...
def insert_fund_name():
    ...
    while i < TOTAL_RECORDS_NUM:
        print('======Starting Request Sina API From Page %s ======'%(PAGE_NUM))
        RECORDS = get_fund_name(PAGE_NUM)
        while j < RECORDS_PER_PAGE:
            print(i,'INSERT INTO TB_FUND_NAME VALUES ',\
                (RECORDS['data'][j]['symbol'],RECORDS['data'][j]['name']))
            DB_CTL.sql_insert_excute(SQL_INSERT_FUND_NAME,\
                (RECORDS['data'][j]['symbol'],\
                    RECORDS['data'][j]['name'],\
                    RECORDS['data'][j]['jjjl']))
